...
| :--- | :--- | :--- |
| [`c704996`](https://github.com/strangeones/yt-jellyfin-downloader/commit/c704996) | **Feature: Channels Only Filter Toggle** | Added an elegant toggle switch to the search bar UI to append `&type=channel` (mapping to `&sp=EgIQAg%3D%3D`) to force YouTube to only return channels in the search results. |
| [`118a03c`](https://github.com/strangeones/yt-jellyfin-downloader/commit/118a03c) | **Fix: Increase Channel Videos Cap to 500** | Replaced the manual HTML `ytInitialData` scraper for the `/api/channel/videos` endpoint with a blazing fast `subprocess` call to `yt-dlp --flat-playlist` to bypass the initial 30-video HTML cap, enabling scrolling through up to 500 old videos. |
| [`014eff9`](https://github.com/strangeones/yt-jellyfin-downloader/commit/014eff9) | **Channel Drill-Down & Multi-Select Queueing** | Built a complete drill-down UI for clicking channels in search results, featuring "Videos" and "Playlists" sub-tabs with in-memory caching. Added a massive multi-select batch queueing architecture with tri-state checkboxes and atomic POST `/api/download` ingestion for multiple videos. |
